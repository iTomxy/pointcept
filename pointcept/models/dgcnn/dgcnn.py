...
__all__ = ["DGCNN_semseg"]


def knn(x, k):
    """Find the index of kNN of each point.
    Args:
        x: float[bs, #channels, #points]
        k: int, #neighbours
    Returns:
        idx: int[bs, #points, k]
    """
...
def get_graph_feature(x, k=20, idx=None, channel_idx=None):
    """
    About the original `dim9` argument:
    - [1] https://github.com/antao97/dgcnn.pytorch/issues/2
    - [2] https://github.com/antao97/dgcnn.pytorch/issues/46
    Now (11 Dec 2025) iTom changes `dim9` to `channel_idx` for a flexible argument
    to suit different input channels.
    For example, as in [2], suppose the raw input has channels
        (X, Y, Z, R, G, B, nX, nY, nZ),
    and you want to use only (nX, nY, nZ) in the 1st call, you can set
        coord_idx=[6,7,8]

    Args:
        x: float[bs, #channels, #points]
        k: int = 20, #neighbours
        idx: int[bs, #points, k] = None, neighbour indices
        channel_idx: int[] = None, indicate which channels to use in finding kNN
    Returns:
        feature: float[bs, 2 * #channels, #points, k]
    """
    batch_size = x.size(0)
    num_points = x.size(2)
...
class DGCNN(nn.Module):
    ...
    """
    def __init__(self, in_channels=3, k=20, emb_dims=1024, out_channels=256, coord_channels=None):
    ...
    def forward(self, input_dict):
        ...
        x = get_graph_feature(x, k=self.k, channel_idx=self.coord_channels)
...
class DGCNN_semseg(nn.Module):
    def __init__(self, num_classes, npoints,
        in_channels=3, k=20, emb_dims=1024, feat_dims=256, dropout=0.5, coord_channels=None,
        criteria=[
        ...
        self.in_channels = in_channels
        self.backbone = DGCNN(in_channels, k, emb_dims, feat_dims, coord_channels)
...
class DGCNN_clreg(nn.Module):
    ...
    def __init__(self, npoints,
        in_channels=3, k=20, emb_dims=1024, feat_dims=256, coord_channels=None,
...
@MODELS.register_module()
class DGCNN_semseg2(nn.Module):
    """Mimic `DGCNN_partseg`: adopt `Transform_Net`"""

    def __init__(self, num_classes, in_channels=3, k=40, emb_dims=1024, dropout=0.5, coord_channels=None):
        """
        Args:
            num_classes: int
            in_channels: int = 3, input dimension, default 3 means xyz
            ...
            coord_channels: int[] = None, indicate which channels are coordinates.
                These channels will be used to find kNN in the 1st call of `get_graph_feature`.
        """
        super(DGCNN_semseg2, self).__init__()
        self.k = k
        self.coord_channels = coord_channels
        self.transform_net = Transform_Net(in_channels)
        if in_channels > 3 and not coord_channels:
            warnings.warn("{}: `in_channels`({}) > 3, but `coord_channels`({}) not set.".format(
                self.__class__.__name__, in_channels, coord_channels))
        ...
        #                            self.bn7,
        #                            nn.LeakyReLU(negative_slope=0.2))
        self.conv8 = nn.Sequential(nn.Conv1d(emb_dims + 64 * 3, 256, kernel_size=1, bias=False),
                                   self.bn8,
                                   nn.LeakyReLU(negative_slope=0.2))
        self.dp1 = nn.Dropout(p=dropout)
        ...
        self.conv10 = nn.Sequential(nn.Conv1d(256, 128, kernel_size=1, bias=False),
                                   self.bn10,
                                   nn.LeakyReLU(negative_slope=0.2))
        self.conv11 = nn.Conv1d(128, num_classes, kernel_size=1, bias=False)
    ...
    def forward(self, input_dict):
        ...
        x0 = get_graph_feature(x, k=self.k,     # (batch_size, 3, num_points) -> (batch_size, 3*2, num_points, k)
            channel_idx=self.coord_channels)    # NOTE only at 1st call
        ...
        logit = self.conv11(x)                  # (batch_size, 256, num_points) -> (batch_size, seg_num_all, num_points)
