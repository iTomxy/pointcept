...
class ClsEvaluator(HookBase):
    ...
    def eval(self):
        self.trainer.logger.info(">>>>>>>>>>>>>>>> Start Cls Evaluation >>>>>>>>>>>>>>>>")
...
class SemSegEvaluator(HookBase):
    ...
    def before_train(self):
        ...
        # (21 Sept 2025, iTom) use an individual logger
        if comm.is_main_process():
            self.json_logger = get_logger(
                "semseg_json_val",
                log_file=os.path.join(self.trainer.cfg.save_path, "semseg_val.json"),
                file_mode="a",
                fmt="%(message)s"
            )
    ...
    def eval(self):
        self.trainer.logger.info(">>>>>>>>>>>>>>>> Start Sem Seg Evaluation >>>>>>>>>>>>>>>>")
        self.trainer.model.eval()
        for i, input_dict in enumerate(self.trainer.val_loader):
            for key in input_dict.keys():
                if isinstance(input_dict[key], torch.Tensor):
...
@HOOKS.register_module()
class SemSegVolumeEvaluator(HookBase):
    """Multi-GPU semantic segmentation evaluator for volume-wise patch datasets.

    Expected validation dataset:
    - `cfg.data.val` should build to a loader-like dataset whose `__getitem__`
      returns a per-volume dataset (e.g. `Ribsegv2VolumeLoader`).
    - The per-patch samples should include `segment` and `voxel_index`.
    """

    def __init__(self, write_cls_iou=False, merge_by_voxel=True):
        self.write_cls_iou = write_cls_iou
        self.merge_by_voxel = merge_by_voxel
        self.val_volume_dataset = None

    def before_train(self):
        if self.trainer.writer is not None and self.trainer.cfg.enable_wandb:
            wandb.define_metric("val/*", step_metric="Epoch")

        self.val_volume_dataset = build_dataset(self.trainer.cfg.data.val)

        if comm.is_main_process():
            self.json_logger = get_logger(
                "semseg_volume_json_val",
                log_file=os.path.join(self.trainer.cfg.save_path, "semseg_volume_val.json"),
                file_mode="a",
                fmt="%(message)s",
            )
    ...
    def _local_volume_ids(self):
        if hasattr(self.val_volume_dataset, "id_list"):
            volume_ids = list(self.val_volume_dataset.id_list)
        else:
            volume_ids = list(range(len(self.val_volume_dataset)))
        return volume_ids[comm.get_rank() :: comm.get_world_size()]

    def _build_volume_loader(self, volume_dataset):
    ...
    @staticmethod
    def _move_to_cuda(input_dict):
        for key in input_dict.keys():
            if isinstance(input_dict[key], torch.Tensor):
                input_dict[key] = input_dict[key].cuda(non_blocking=True)
        return input_dict

    def _predict_volume(self, volume_loader):
        logits_list, label_list, index_list = [], [], []
        loss_sum = 0.0
        loss_count = 0
        for batch in volume_loader:
            batch = self._move_to_cuda(batch)
            with torch.no_grad():
                output_dict = self.trainer.model(batch)

            logits_list.append(output_dict["seg_logits"].detach().cpu().numpy())
            label_list.append(batch["segment"].detach().cpu().numpy())
            index_list.append(batch["voxel_index"].detach().cpu().numpy().astype(np.int64))
            loss_sum += float(output_dict["loss"].item())
            loss_count += 1
        ...
        return logits, label, voxel_index, loss_sum, loss_count

    @staticmethod
    def _merge_by_voxel_index(logits, label, voxel_index):
        unique_index, first_idx, inverse = np.unique(
            voxel_index, axis=0, return_index=True, return_inverse=True
        )
        ...
        return logits_merged, label_merged, unique_index

    def eval(self):
        self.trainer.logger.info(">>>>>>>>>>>>>>>> Start Sem Seg Volume Evaluation >>>>>>>>>>>>>>>>")
        self.trainer.model.eval()
        assert self.val_volume_dataset is not None

        local_tp = torch.zeros(self.trainer.cfg.data.num_classes, dtype=torch.long, device="cuda")
        local_tn = torch.zeros_like(local_tp)
        local_fp = torch.zeros_like(local_tp)
        local_fn = torch.zeros_like(local_tp)
        loss_sum = torch.zeros(1, dtype=torch.float64, device="cuda")
        loss_count = torch.zeros(1, dtype=torch.float64, device="cuda")
        volume_count = 0

        for volume_id in self._local_volume_ids():
            volume_dataset = self.val_volume_dataset[volume_id]
            volume_loader = self._build_volume_loader(volume_dataset)
            logits, label, voxel_index, volume_loss_sum, volume_loss_count = self._predict_volume(volume_loader)
            if self.merge_by_voxel:
                logits, label, voxel_index = self._merge_by_voxel_index(logits, label, voxel_index)

            pred = logits.argmax(1)
            tp, tn, fp, fn = confusion_matrix(
                torch.from_numpy(pred).cuda(non_blocking=True),
                torch.from_numpy(label).cuda(non_blocking=True),
                self.trainer.cfg.data.num_classes,
                self.trainer.cfg.data.ignore_index,
            )
            local_tp += tp
            ...
            local_fp += fp
            local_fn += fn
            loss_sum += volume_loss_sum
            loss_count += volume_loss_count
            volume_count += 1

            self.trainer.logger.info(
                "Val volume {volume_id} [{done}] loss {loss:.4f}".format(
                    volume_id=getattr(volume_dataset, "volume_id", volume_id),
                    done=volume_count,
                    loss=volume_loss_sum / max(volume_loss_count, 1),
                )
            )

        comm.synchronize()
        if comm.get_world_size() > 1:
            for tensor in (local_tp, local_tn, local_fp, local_fn, loss_sum, loss_count):
                dist.all_reduce(tensor, op=dist.ReduceOp.SUM)

        tp = local_tp.cpu().numpy()
        tn = local_tn.cpu().numpy()
        fp = local_fp.cpu().numpy()
        fn = local_fn.cpu().numpy()
        metrics = calc_cm_metrics(
            tp=tp,
            tn=tn,
            fp=fp,
            fn=fn,
            class_set=self.trainer.cfg.data.num_classes,
            ignore_cls=[self.trainer.cfg.data.bg_class],
        )
        loss_avg = float((loss_sum / torch.clamp(loss_count, min=1)).item())
        m_iou = metrics["iou"]
        m_acc = metrics["acc_macro"]
        all_acc = metrics["acc_micro"]
        iou_class = np.asarray(metrics["iou_class"])
        acc_class = np.asarray(metrics["acc_class"])
        ...
        if comm.is_main_process():
            ...
            self.json_logger.info(json.dumps({
                "epoch": current_epoch,
                "loss": loss_avg,
                "iou": m_iou,
                "acc_macro": m_acc,
...
class InsSegEvaluator(HookBase):
    ...
    def before_train(self):
        ...
        if comm.is_main_process():
            self.json_logger = get_logger(
                "insseg_json_val",
                log_file=os.path.join(self.trainer.cfg.save_path, "insseg_val.json"),
                file_mode="a",
                fmt="%(message)s"
            )
    ...
    def eval(self):
        self.trainer.logger.info(">>>>>>>>>>>>>>>> Start Ins Seg Evaluation >>>>>>>>>>>>>>>>")
        self.trainer.model.eval()
        scenes = []
        for i, input_dict in enumerate(self.trainer.val_loader):
            assert (
